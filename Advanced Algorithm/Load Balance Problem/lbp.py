...
import numpy as np
import pulp  # 导入 pulp库 pip install pulp
from matplotlib import pyplot as plt


def sample():
    lbp = pulp.LpProblem("lbp", sense=pulp.LpMinimize)  # 定义问题，求最大值
    ...
    plt.axis('off')
    if is_cycle:
        plt.savefig("net_with_cycle.jpg")
    else:
        plt.savefig("net_without_cycle.jpg")
    # plt.show()
    plt.clf()
    return is_cycle
